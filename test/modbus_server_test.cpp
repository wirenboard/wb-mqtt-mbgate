...
MATCHER_P(Pointee16, value, "")
{
    return *(static_cast<const uint16_t*>(arg)) == value;
}
MATCHER_P2(Pointee16_2, value1, value2, "")
{
    auto p = static_cast<const uint16_t*>(arg);
    return p[0] == value1 && p[1] == value2;
}
MATCHER_P(Pointee8, value, "")
{
    return *(static_cast<const uint8_t*>(arg)) == value;
}
...
TEST_F(ModbusServerTest, AllocationTest)
{
    ...
    EXPECT_CALL(*obs1, OnCacheAllocate(COIL, 1, cache_range1)).Times(1);
    EXPECT_CALL(*obs2, OnCacheAllocate(COIL, 1, cache_range2)).Times(1);
    EXPECT_CALL(*obs3, OnCacheAllocate(INPUT_REGISTER, 2, cache_range3)).Times(1);
